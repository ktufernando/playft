/**
 * clients.create.step5.controllers.js
 *
 * PLAYFT.
 *
...
angular.module(ApplicationConfiguration.applicationModuleName).controller('ClientsCreateStep5Controller',
    function ($scope, $state, CommonService, $stateParams, growl,
                Messages, $timeout, Clients, ModalService, MatrixService) {
        ...
        if(!$scope.client.bankAccounts){
            $scope.client.bankAccounts = [];
        }

        $scope.remove = function(index){
            $scope.client.bankAccounts.splice(index, 1);
        };
        ...
        $scope.next = function(){
            if(!$scope.client.bankAccounts.length){
                ...
                Clients.create($scope.client, function (r) {
        ...
        $scope.modalUpdateMatrix = function(){
            ModalService.showModal('warning', Messages.WARNING,
                '¿Quiere ejecutar el cálculo de la matriz del cliente agregado? <br> <br> <strong>Aclaración:</strong> Las matrices se calculan automáticamente todos los días.')
                .result.then(function (a) {
                MatrixService.matrixForAll(
                    function(resp){
                        growl.success('Las matriz se calculó con éxito')
                    },
                    function(error){
                        growl.error(error.data.statusMessage);
                    });
